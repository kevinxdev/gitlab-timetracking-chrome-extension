            ...
            fetch(request)
                .then(response => {
                    if (gitlabUrlInput.classList.contains('is-invalid')) {
                        ...
                        document.getElementById("validationServer03Feedback").classList.add("invisible");
                    }
                })
                .catch(error => {
                    gitlabUrlInput.classList.add("is-invalid");
                    document.getElementById("validationServer03Feedback").classList.remove("invisible");
                })
        });
                ...
                fetch(request)
                    .then(response => {
                        if (gitlabPATInput.classList.contains('is-invalid')) {
                            ...
                            document.getElementById("validationServer04Feedback").classList.add("invisible");
                        }
                        response.json().then(data => {
                            if (data.message == "401 Unauthorized") {
                                gitlabPATInput.classList.add("is-invalid");
                                document.getElementById("validationServer04Feedback").classList.remove("invisible");
                            } else {
